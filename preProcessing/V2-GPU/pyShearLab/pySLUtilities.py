...
def SLcheckFilterSizes(rows,cols, shearLevels,directionalFilter,scalingFilter,
    ...
    h0 = np.array(h0, dtype=object) / np.sqrt(2)
    h1 = np.array(h1, dtype=object) / np.sqrt(2)
...
###########################################################################
    ...
    for j in range(len(filterHigh)-2,-1,-1):
        ...
        print("filterHigh[{}] numpy: mean={}, std={}".format(j, np.mean(filterHigh[j]), np.std(filterHigh[j])))
    ...
    for shearLevel in np.unique(shearLevels):
            ...
            h_np, w_np = wedgeHelp.shape
            cy_np, cx_np = h_np // 2, w_np // 2
            print("[NUMPY] whelp L=", L, f"shape=({h_np},{w_np}) center={wedgeHelp[cy_np, cx_np]:.6f}")

            print(f"[DEBUG PY] wedge_help[{shearLevel}] shape: {wedgeHelp.shape}")
            center_y, center_x = wedgeHelp.shape[0] // 2, wedgeHelp.shape[1] // 2
            print(f"[DEBUG PY] wedge_help[{wedgeHelp}] center value: {wedgeHelp[center_y, center_x]}")

            wedgeHelp = SLpadArray(wedgeHelp,np.array([rows,cols]));
            print("[NUMPY] wpad       shape=({},{}) center={:.6f}".format(rows, cols, wedgeHelp[rows//2, cols//2]))
            ...
            print(f"[DEBUG PY] wedge_upsampled[{shearLevel}] shape: {wedgeUpsampled.shape}")
            ...
            print(f"[DEBUG PY]: lowpass is never padded ")
    ...
    print("NumPy bandpass[0,0,1]:", bandpass[0,0,1]) 
